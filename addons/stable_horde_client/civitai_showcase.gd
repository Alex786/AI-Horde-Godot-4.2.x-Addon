...
@export var showcase_index: int = 0
...
func _on_request_completed(_result, response_code, _headers, body):
	if response_code == 0:
		var error_msg: String = "Model showcase address cannot be resolved!"
		push_error(error_msg)
		return
	if response_code == 404:
		var error_msg: String = "Bad showcase URL. Please contact the developer of this addon"
		push_error(error_msg)
		return
	var image: Image = Image.new()
	var image_error: Error = image.load_webp_from_buffer(body)
	if image_error != OK:
